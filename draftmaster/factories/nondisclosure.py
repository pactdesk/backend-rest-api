...
class NondisclosureFactory:
    ...
    def _generate_parties(self) -> Section:
        ...
        section.closing = self._load_template(
    ...
    def _generate_agreements(self) -> Section:
        ...
        if self.request.penalty_clause:
            ...
            enforcement_clause = Clause(**self._load_template(
                enforcement_path, "enforcement_and_penalties.json"
            ))
        else:
            enforcement_clause = Clause(**self._load_template(
                clauses_path, "enforcement_and_remedies.json"
            ))
        
        no_warranty_index = next(
            i for i, clause in enumerate(clauses) 
            if clause.title.lower().startswith("no warranty")
        )
